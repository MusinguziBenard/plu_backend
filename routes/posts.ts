...
// routes/posts.ts — FINAL v18: ALL LIKES COUNTED + FULL INTEGRITY
import { Router } from 'express'
import { requireAuth } from '../middleware/auth'
import { Post } from '../models/Post'
...
// PUBLIC FEED — COUNT ALL LIKES + USER LIKE STATUS
router.get('/feed', async (req: any, res) => {
  try {
    const userId = req.user?.id // Optional: user may be logged in

    // 1. Get all posted posts with total likes count
    const posts = await Post.findAll({
      where: { status: 'posted' },
      include: [{
        model: User,
      ...
      attributes: {
        include: [
          // COUNT ALL LIKES FROM post_likes TABLE
          [Post.sequelize!.fn('COUNT', Post.sequelize!.col('PostLikes.id')), 'total_likes']
        ]
      },
      include: [{
        model: PostLike,
    ...
    // 2. For each post, check if current user liked it
    const postsWithLikeStatus = await Promise.all(
      posts.map(async (post: any) => {
        ...
        return {
          ...post.toJSON(),
          likes_count: parseInt(post.getDataValue('total_likes')) || 0,
          is_liked: isLiked
        }
      })
    )
