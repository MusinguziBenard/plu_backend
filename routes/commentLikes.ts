...
// routes/commentLikes.ts - ORIGINAL CODE + SOCKET.IO ONLY
import { Router } from 'express'
import { requireAuth } from '../middleware/auth'
import { CommentLike } from '../models/CommentLike'
import { Comment } from '../models/Comment'
...
router.post('/toggle', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      ...
      // ✅ SOCKET.IO - Emit unlike event
      const io = req.app.get('io')
      if (io) {
        const comment = await Comment.findByPk(comment_id, { attributes: ['post_id'] })
        if (comment?.post_id) {
          io.to(`post:${comment.post_id}`).emit('comment_like_toggled', {
            commentId: comment_id,
      ...
      // ✅ SOCKET.IO - Emit like event
      const io = req.app.get('io')
      if (io && comment?.post_id) {
        io.to(`post:${comment.post_id}`).emit('comment_like_toggled', {
          commentId: comment_id,
          postId: comment.post_id,
          userId,
