...
// routes/notifications.ts - WITH SOCKET.IO (ORIGINAL CODE PRESERVED)
import { Router } from 'express'
import { requireAuth } from '../middleware/auth'
import { Notification } from '../models/Notification'
import { UserPushToken } from '../models/UserPushToken'
...
router.patch('/:id/read', async (req, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit notification read
...
router.post('/test', async (req, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit test notification
    const io = req.app.get('io')
    if (io) {
...
router.post('/admin/broadcast', async (req: any, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit broadcast to all connected users
    const io = req.app.get('io')
    if (io) {
