...
// routes/savedPosts.ts - ORIGINAL CODE + SOCKET.IO ONLY
import { Router } from 'express'
import { requireAuth } from '../middleware/auth'
import { SavedPost } from '../models/SavedPost'
import { Post } from '../models/Post'
...
router.post('/save/:postId', async (req, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit save event
    const io = req.app.get('io')
    if (io) {
      io.to(`post:${postId}`).emit('post_saved', { postId, userId, timestamp: Date.now() })
      io.to(`user:${userId}`).emit('saved_posts_updated', { type: 'save', postId, timestamp: Date.now() })
    }
...
router.delete('/unsave/:postId', async (req, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit unsave event
    const io = req.app.get('io')
    if (io) {
      io.to(`post:${postId}`).emit('post_unsaved', { postId, userId, timestamp: Date.now() })
      io.to(`user:${userId}`).emit('saved_posts_updated', { type: 'unsave', postId, timestamp: Date.now() })
    }
