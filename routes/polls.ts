...
// routes/polls.ts - ORIGINAL CODE + SOCKET.IO ONLY
import { Router } from 'express'
import { requireAuth } from '../middleware/auth'
import { Poll } from '../models/Poll'
import { PollOption } from '../models/PollOption'
...
protectedRouter.post('/create', async (req, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit poll created
    if (postId) {
      const io = req.app.get('io')
      if (io) {
        io.to(`post:${postId}`).emit('poll_created', {
          pollId: poll.id,
          postId,
          question,
...
protectedRouter.post('/vote/:pollId', async (req, res) => {
  try {
    ...
    if (poll.post_id) {
      const io = req.app.get('io')
      if (io) {
        io.to(`post:${poll.post_id}`).emit('poll_vote', {
          pollId,
          postId: poll.post_id,
          userId,
...
protectedRouter.patch('/end/:pollId', async (req, res) => {
  try {
    ...
    // ✅ SOCKET.IO - Emit poll ended
    if (poll.post_id) {
      const io = req.app.get('io')
      if (io) {
        io.to(`post:${poll.post_id}`).emit('poll_ended', {
          pollId,
          postId: poll.post_id,
          timestamp: Date.now()
        })
